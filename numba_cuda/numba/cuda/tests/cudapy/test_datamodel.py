...
from numba.cuda.core.datamodel.testing import test_factory
from numba.cuda.core.datamodel.manager import DataModelManager
...
class TestArgInfo(unittest.TestCase):
    def _test_as_arguments(self, fe_args):
        ...
        args = [
            ir.Constant(dmm.lookup(t).get_value_type(), None) for t in fe_args
        ]
...
class TestDMMChaining(CUDATestCase):
    def test_basic(self):
