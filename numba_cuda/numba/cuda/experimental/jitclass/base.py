...
from numba.core import errors, imputils, types
from numba.cuda import cgutils, utils
from numba.core.datamodel import default_manager, models
from numba.core.registry import cpu_target
from numba.core.typing import templates
from numba.core.typing.asnumbatype import as_numba_type
from numba.cuda.serialize import disable_pickling
from numba.cuda.experimental.jitclass import _box
...
def _validate_spec(spec):
    for k, v in spec.items():
        ...
        if not isinstance(v, types.Type):
            raise TypeError(
                "spec values should be Numba type instances, got %r" % (v,)
            )
...
def _drop_ignored_attrs(dct):
    ...
    for k, v in dct.items():
        if isinstance(
            v, (pytypes.BuiltinFunctionType, pytypes.BuiltinMethodType)
        ):
            drop.add(k)
...
class ClassBuilder(object):
    ...
    def register(self):
        ...
        self._register_methods(
            self.class_impl_registry, self.class_type.instance_type
        )
...
def get_attr_impl(context, builder, typ, value, attr):
    ...
    if attr in typ.struct:
        ...
        data = context.make_data_helper(
            builder, typ.get_data_type(), ref=data_pointer
        )
...
def set_attr_impl(context, builder, sig, args, attr):
    ...
    if attr in typ.struct:
        ...
        data_ptr = inst.data
        data = context.make_data_helper(
            builder, typ.get_data_type(), ref=data_ptr
        )
...
def imp_dtor(context, module, instance_type):
    ...
    dtor_ftype = llvmir.FunctionType(
        llvmir.VoidType(), [llvoidptr, llsize, llvoidptr]
    )
...
@ClassBuilder.class_impl_registry.lower(
    types.ClassType, types.VarArg(types.Any)
)
